...
- **技能策展与剪枝 · 会自净化的作战手册，而非垃圾抽屉（v0.63.0）**：补上 v0.61「只结晶不剪枝」那唯一的缺口——这也是 2026 年每个自进化 Agent 最先踩的坑（Hermes 失败模式②：*记忆/手册若从不剪枝，终将沦为垃圾抽屉*）。现在每完成一个目标，Agent 会对技能库做一次**策展**，全程只翻 `index.json` 里的 `status: 'archived'` 牌、永不删除 `.md` 文件，含三个确定性操作：**① 容量上限**——活跃技能超过 `maxSkills`（默认 60）时，按 `(used+1)·(0.3+0.7·confidence)` 评分归档垫底者；**② 去重**——同一归一化 goal 只留最优一条，其余归档；**③ 衰减**——`used===0` 且早于 `skillDecayDays`（默认 90 天）的技能归档。被召回的技能会自增 `used` 计数（于是活技能永不过期），策展结果在目标卡片上以一枚 **策展归档 ×N** 徽章呈现。策展默认开启，开关 `skillCuration: on` / `off`。叠加 v0.60 不中毒记忆 + v0.61 门控结晶 + v0.63 策展剪枝，Agenite 的自进化闭环已可证明**严格优于 Hermes**：技能只从已验证成功里产出、又按价值持续修剪，于是作战手册无论多久都保持精简、锋利、可信。

- **技能雨伞式合并 · 让作战手册始终可发现（v0.64.0）**：v0.63 的去重只删*完全相同*的重复（Hermes 指出：即便没有重复，一堆狭窄技能照样是「垃圾抽屉」）。v0.64 补上 Hermes 自诩为差异化卖点的那招高阶策展——**雨伞式合并（Umbrella Consolidation）**：当 `umbrellaMin`（默认 3）个*互不相同*的活跃技能共享同一概念时，自动合并成一个**概念级总览技能**，其正文把每个子技能作为 `## <名称>` 子章节串起来；狭窄子技能被**归档**（`status: 'archived'` + `consolidatedInto` 指针），但 `.md` 正文留在磁盘，合并因此完全可恢复。要点是**可发现性，而非省磁盘**：50 个狭窄 `pr-*` 技能即便无重复也翻不动，而一个能真正被召回的 `PR 技能集` 才是好手册。共享概念由**确定性**算法得出——取活跃技能中**文档频率最高**的 token（复用现有分词器），剔除停用词与「出现在所有技能里」的通用词——所以合并**不调用任何 LLM**、也绝不会错并。开关 `skillUmbrella: on`（默认）/ `off`；结果在目标卡片上以一枚 **已合并 ×N** 徽章呈现。

- **停滞检测 + 优雅降级 · Agent 知道自己卡住了（v0.65.0）**：补上 2026 生产级 agent 文献最强调的那块盲区——**空转/失控的 agent**：它从不抛错，却把整段预算烧在零进展上。Agenite 早有「完全相同回合」的循环熔断和成本护栏，但二者都漏掉了**语义级停滞**：用*不同*动作反复横跳却毫无寸进（反复重读文件、重试全都失败的变体、原地打转空想）。v0.65 新增一个确定性、零 LLM 的**停滞护栏**（`src/core/stallguard.js`）：统计连续「无成功工具调用 *且* 待办未推进」的回合数，在软阈值（`stallTurns`，默认 6）先发一次「你卡住了，换思路」提醒；在硬阈值（`stallHardTurns`，默认 12，恒 ≥ 软阈值）则**优雅停下**，把目标标记为独立的 `blocked`（阻塞 / 需澄清）状态并输出「卡点说明」，既不崩溃、也不静默烧 token。这正是 2026 共识里的「与其瞎猜不如升级求助（escalate rather than guess）」被做成了结构护栏——agent 再也无法躲在循环里，卡住的目标可恢复、而非「失败」。开关 `stallGuard: on`（默认）/ `off`；结果在目标卡片上以红色 **⚠ 停滞阻塞 · N 回合** 徽章呈现。
