...
// v0.72: keep one in-place notice that tells the whole reversible-compression
// story — how many results were shrunk, how many tokens that saved, and how
// many times the model actually pulled an original back via context_retrieve.
// The user sees a closed loop ("compressed → retrieved"), not two orphan chips.
function renderEconomyNotice(el, aMsg) {
  const e = aMsg.shrink || { n: 0, saved: 0, retrieved: 0, kinds: {} };
  const parts = [];
  if (e.n > 0) parts.push(`⧉ 上下文压缩 ${e.n} 次，省约 ${fmtTokens(e.saved)} tokens（原文可取回）`);
  if (e.retrieved > 0) parts.push(`🔍 已取回 ${e.retrieved} 次（缓存命中，未重跑原工具）`);
  if (!parts.length) return;
  const text = parts.join('；');
  const detail = (Object.entries(e.kinds || {}).map(([k, v]) => `${k} ×${v}`).join('\n') || '（无压缩明细）') +
  ...
  try {
    await postStream('/api/chat', {
      ...
      if (event === 'delta') {
        ...
        // loop. Track it on the same economy notice as compression so the user
        // sees "compressed → retrieved" as one story.
        if (data.name === 'context_retrieve') {
          aMsg.shrink = aMsg.shrink || { n: 0, saved: 0, retrieved: 0, kinds: {} };
          aMsg.shrink.retrieved = (aMsg.shrink.retrieved || 0) + 1;
          renderEconomyNotice(el, aMsg);
        }
        ...
        // v0.71: one guardrail event now covers the budget cap and the action-level
        // blast-radius gate. Toast the three meaningful shapes distinctly.
        if (data?.decision === 'deny') {
          toast('🛡️ 护栏拦截：' + (data.tool || '') + ' 被阻止（' + (data.reason || 'deny') + '）', 4500);
        } else if (data?.decision === 'ask') {
          toast('🛡️ 护栏：' + (data.tool || '') + ' 需审批（' + (data.category || '') + '）', 3500);
        } else {
          // cost (default) — the server hit the interactive budget cap and forced a stop.
        ...
        // v0.69: also surface the structured reason + the fact that counters were
        // reset on a real replan, so the user knows recovery actually executed.
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .tstep-ms { font-size: 12px; opacity: .55; margin-left: auto; } .tstep-bad { color: #c0392b; font-size: 12px; } .tstep-ok { color: #1e8e3e; font-size: 12px; }
...
function traceOnEvent(type, payload) {
  ...
  if (type === 'assistant') {
    ...
    // v0.71: render a trace step only for meaningful decisions (deny / ask / cost).
    // The per-tool 'allow' entries are no-ops for the timeline but still roll the
    // server-side audit ledger, so we skip them here to keep the view readable.
    if (payload?.decision !== 'allow') {
      const label = payload?.decision === 'deny' ? '拦截'
        : payload?.decision === 'ask' ? '待审批'
        : '预算';
      traceAdd({
        kind: 'guardrail',
        name: '护栏·' + label + (payload?.tool ? '·' + payload.tool : ''),
        status: (payload?.decision === 'deny' || payload?.decision === 'cost') ? 'error' : 'ok',
        data: payload || {}
      });
    }
    ...
    // v0.75.0: Plan Self-Refinement. Render a timeline step listing the concrete
    // fix suggestions right after the gate, so the user sees HOW to repair a
    // failing plan without leaving the timeline. Non-blocking toast only when
    // there are error-level (must-fix) suggestions, to avoid noise on clean plans.
    ...
    traceAdd({
      ...
      parentId: traceTurnId || null,
      status: errs ? 'error' : (warns ? 'ok' : 'ok'),
      data: { count: suggestions.length, level, suggestions }
    });
    if (errs) {
      toast('🛠️ 规划自精炼：发现 ' + errs + ' 处必须修复的问题，详见规划时间线', 4200);
    }
  } else if (type === 'plan_decompose') {
    // v0.76.0: Plan Decomposition. Render the seeded draft plan as the first
    // planning step so the user sees the research → action → verify skeleton
    // the run started from. Advisory only — no toast (it is a seed, not a
    // verdict), to keep the timeline informative without adding noise.
...
function traceStepHtml(s, depth) {
  ...
  if (s.kind === 'tool') {
  ...
  } else if (s.kind === 'guardrail') {
    const d = s.data || {};
    const bits = [];
    if (d.decision) bits.push(`判定 <b>${escapeHtml(String(d.decision))}</b>`);
    if (d.category) bits.push(`类别 <b>${escapeHtml(d.category)}</b>`);
    if (d.reason) bits.push(`原因 <b>${escapeHtml(d.reason)}</b>`);
  ...
  } else if (s.kind === 'plan_refine') {
    // v0.75.0: render the concrete fix suggestions so the user can repair the
    // plan. Reuses the same issue-item styling as plan_gate for visual continuity.
    const d = s.data || {};
    const suggestions = Array.isArray(d.suggestions) ? d.suggestions : [];
    const head = `建议 <b>${typeof d.count === 'number' ? d.count : suggestions.length}</b> 条`;
    const items = suggestions.map((i) => {
      const ico = i.severity === 'error' ? '✕' : i.severity === 'warning' ? '⚠' : '·';
      const cls = i.severity === 'error' ? 'fb' : i.severity === 'warning' ? 'fw' : 'fn';
      const step = i.step ? `（第${i.step}步）` : '';
      return `<div class="td-item ${cls}"><span class="tdi-ico">${ico}</span><div class="tdi-title">${escapeHtml((i.message || i.code || '') + step)}</div></div>`;
    ...
    detail = `<div class="tstep-content muted small">${head}</div>` + (items ? `<div class="tstep-issues">${items}</div>` : '');
  } else if (s.kind === 'plan_decompose') {
    // v0.76.0: render the seeded draft plan steps (research → action → verify)
    // so the user can see the skeleton the run started from right in the
    // timeline. Reuses the issue-item styling for visual continuity.
    const d = s.data || {};
    const steps = Array.isArray(d.steps) ? d.steps : [];
    const kindIcon = { research: '🔍', action: '⚙', verify: '✅' };
    const kindLabel = { research: '调研', action: '执行', verify: '验证' };
    const head = `草稿 <b>${typeof d.stepCount === 'number' ? d.stepCount : steps.length}</b> 步`;
    const items = steps.map((i) => {
      const ico = kindIcon[i.kind] || '·';
